...
const RegsApp = () => {
  ...
  const handleUpdateNC = (recordId, updatedData) => {
    setConfirmModal({
      show: true,
      title: '¿Confirmar cambios en el informe?',
      action: async () => {
        const { error } = await supabase
          .from('registros')
          .update({
            datos: updatedData,
            producto: 'No conformidad - ' + updatedData.codigo,
            responsable: updatedData.responsable,
            fecha: formatInputDate(updatedData.fecha),
        ...
        if (!error) {
          await logAction(supabase, activeSector, 'Modificación NC', updatedData.responsable, { recordId, codigo: updatedData.codigo });
          fetchRecords();
          setSelectedRecord(prev => ({ ...prev, ...updatedData, datos: updatedData, estado: updatedData.estado }));
        }
        setConfirmModal({ show: false, action: null, title: '' });
      }
    });
  ...
  const handleCloseNC = (recordId) => {
    setConfirmModal({
      show: true,
      title: '¿Cerrar definitivamente este informe?',
      action: async () => {
        const record = records.find(r => r.id === recordId);
        if (!record) return;

        const updatedDatos = { ...record.datos, estado: 'Cerrado' };
        ...
        if (!error) {
          await logAction(supabase, activeSector, 'Cierre NC', record.responsable, { recordId, codigo: record.codigo });
          fetchRecords();
          setSelectedRecord(prev => ({ ...prev, estado: 'Cerrado', datos: updatedDatos }));
        }
        setConfirmModal({ show: false, action: null, title: '' });
      }
    });
                ...
                <main className="content" style={activeSector === 'sistemas' ? { padding: '0 2rem 1.5rem 2rem' } : {}}>
                  <AnimatePresence mode="wait">
                    ...
                    {(activeSubTab === 'form' || activeSubTab === 'personal') ? (
                      ...
                      (() => {
                        const isQuality = activeSector === 'calidad';
                        const isClosed = selectedRecord.estado === 'Cerrado';
                        const isEditable = isQuality && !isClosed;
                          ...
                          <>
                            ...
                            <div className="report-view-grid">
                              <div className="view-group">
                                ...
                                {isEditable ? (
                                  <select
                                    className="view-value-input highlight"
                                    style={{ color: '#ef4444', fontWeight: '800' }}
                                    value={selectedRecord.datos?.areaImplicada}
                                    onChange={(e) => setSelectedRecord({ ...selectedRecord, datos: { ...selectedRecord.datos, areaImplicada: e.target.value } })}
                                  >
                                    {SECTORS.map(s => (
                                      <option key={s.id} value={s.id}>{s.label}</option>
                                    ))}
                              ...
                              <div className="view-group">
                                ...
                                {isEditable ? (
                                  <input
                                    type="text"
                                    className="view-value-input"
                                    value={selectedRecord.datos?.fecha}
                                    onChange={(e) => setSelectedRecord({ ...selectedRecord, datos: { ...selectedRecord.datos, fecha: handleDateMask(e.target.value, selectedRecord.datos?.fecha) } })}
                                  />
                                ) : (
                                  <div className="view-value">{formatInputDate(selectedRecord.fecha)}</div>
                                )}
                            ...
                            <div className="view-group full">
                              ...
                              {isEditable ? (
                                <textarea
                                  className="view-value-textarea large"
                                  value={selectedRecord.datos?.descripcion}
                                  onChange={(e) => setSelectedRecord({ ...selectedRecord, datos: { ...selectedRecord.datos, descripcion: e.target.value } })}
                                />
                              ) : (
                            ...
                            <div className="view-group full">
                              ...
                              {isEditable ? (
                                <textarea
                                  className="view-value-textarea large"
                                  value={selectedRecord.datos?.causaRaiz}
                                  onChange={(e) => setSelectedRecord({ ...selectedRecord, datos: { ...selectedRecord.datos, causaRaiz: e.target.value } })}
                                />
                              ) : (
                            ...
                            <div className="view-group full">
                              ...
                              {isEditable ? (
                                <textarea
                                  className="view-value-textarea large"
                                  value={selectedRecord.datos?.accionCorrectiva}
                                  onChange={(e) => setSelectedRecord({ ...selectedRecord, datos: { ...selectedRecord.datos, accionCorrectiva: e.target.value } })}
                                />
                              ) : (
                            ...
                            <div className="report-view-footer" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                              <div className="view-group">
                                <label>Responsable</label>
                                {isEditable ? (
                                  <input
                                    type="text"
                                    className="view-signature-input"
                                    value={selectedRecord.datos?.responsable}
                                    onChange={(e) => setSelectedRecord({ ...selectedRecord, datos: { ...selectedRecord.datos, responsable: e.target.value } })}
                                  />
                                ) : (
                                  <div className="view-signature">{selectedRecord.responsable}</div>
                                )}
                              ...
                              {isEditable && (
                                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                                  <button
                                    className="save-nc-changes-btn"
                                    onClick={() => handleUpdateNC(selectedRecord.id, selectedRecord.datos)}
                                  >
                                    Guardar Cambios
                                  </button>
                                  <button
                                    className="close-nc-btn"
                                    onClick={() => handleCloseNC(selectedRecord.id)}
                                  >
                                    Cerrar
                                  </button>
                                </div>
                            ...
                            <div className="view-group full" style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '2px solid rgba(255,255,255,0.05)' }}>
                              ...
                              >
                                ...
                                {/* New Input Area */}
                                {(activeSector === selectedRecord.sector || activeSector === getSectorFromAreaImplicada(selectedRecord.areaImplicada)?.id) && !isClosed ? (
                                  <div style={{ display: 'flex', alignItems: 'flex-start', flex: 1 }}>
                                    <span style={{ color: SECTORS.find(s => s.id === activeSector)?.color || '#fff', fontWeight: '900', marginRight: '0.5rem', textTransform: 'uppercase', fontSize: '0.8rem', paddingTop: '1px' }}>
                                      {SECTORS.find(s => s.id === activeSector)?.label || 'SECTOR'}:
                                    </span>
                                  ...
                                  <div style={{ color: '#666', fontSize: '0.85rem', fontStyle: 'italic', marginTop: selectedRecord.respuestas?.length ? 0 : '1rem' }}>
                                    {isClosed ? "Este informe está cerrado y no admite más respuestas." : (selectedRecord.respuestas?.length === 0 ? "No hay respuestas registradas aún." : "El área implicada responderá aquí.")}
                                  </div>
                                )}
                              </div>
